...
TERMS = ("bond", "angle", "chiral", "cistrans", "vdw", "plane", "torsion")
...
@pytest.mark.parametrize(
    "block", [{}, {"plane": {}}, {"torsion": {}}, {"start_sigma": 1.0}]
)
...
@pytest.mark.parametrize(
    ...
    [
        ({}, False),
        ({"torsion": {}}, False),
        ({"cistrans": {"weight": 0}}, False),
        ({"torsion": {"weight": 1}, "cistrans": {"weight": 0}}, True),
    ],
)
def test_polymer_torsions_are_independent_and_opt_in(config, enabled):
    from rgi_toolkit.polymer import build_polymer_geometry
    from tests.test_conformer_chemistry import _peptide

    adapter, _ = _peptide("SFR")
    geometry = build_polymer_geometry(adapter, config)
    assert bool(geometry.library.terms["torsion"]) == enabled
    spec = build_spec(
        conformer_config=config,
        polymer_geometry=geometry,
        elements=adapter.get_elements(),
    )
    assert (spec.torsion is not None) == enabled
    assert spec.cistrans is None


def _geometry(torsions):
    library = LibraryTargets()
    library.terms["torsion"] = torsions
...
def test_only_conflicting_groups_removed_and_zero_weight_restores_them():
    ...
    config = {key: {"weight": int(key in ("plane", "torsion"))} for key in TERMS}
...
def test_plane_survives_only_in_nonconflicting_peptide_state(source):
    ...
    for degrees, plane_mask in ((0, 0), (180, 1)):
        ...
        assert bound["torsion"]["mask"].tolist() == [1 - plane_mask]


def test_condition_subtraction_is_local_and_disjoint():
    library = LibraryTargets()
    library.terms["torsion"] = [
        GeometryTarget((0, 1, 2, 3), 0, 0.1, conditions=((2, 0), (7, 1)))
    ]
    planes, conditions, _ = prefer_torsions([(0, 1, 2, 3, 4)], [], library)
    assert len(planes) == 2
    for a in (0, 1):
        for b in (0, 1):
            state = {2: a, 7: b}
