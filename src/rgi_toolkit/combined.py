...
from rgi_toolkit.featurizer import _conf_weight, build_spec
...
def _enable_verbose_logging() -> None:
    ...
    if not any(isinstance(h, logging.StreamHandler) for h in pkg.handlers):
        ...
        handler.setFormatter(
            logging.Formatter("[rgi_toolkit] %(levelname)s %(message)s")
        )
...
class CombinedRestraints:
    ...
    def setup(self, adapter, nbatch: int = 1, config: dict | None = None) -> None:
        ...
        if not self.spec.is_active():
            ...
            if cfg.verbose:
                ...
                print("[rgi_toolkit] setup: NO ACTIVE RESTRAINTS", flush=True)
            return
        ...
        if cfg.verbose:
            ...
            if cfg.base_pair_data:
                ...
                bp_msg = (
                    f"[rgi_toolkit] setup: base_pair={len(cfg.base_pair_data)} entries "
    ...
    def _warn_never_active(self) -> None:
        ...
        for m in msgs:
            logger.warning(m)
            if self.config.verbose:
                print(f"[rgi_toolkit] WARNING: {m}", flush=True)
        if errors:
    ...
    def finalize(self, coords, istep: int = 0) -> None:
        ...
        try:
            ...
            total += dyn_vdw
            msg = (
                f"[rgi_toolkit] finalize (step {istep}): "
